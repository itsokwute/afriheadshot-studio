...
export async function runHeadshotGeneration(config: GenerationConfig): Promise<GeneratedResult> {
  ...
  // 1. Try Real AI Generation via Replicate API if token is configured
  if (token) {
    try {
 ...
 * with professional Rembrandt lighting, attire overlay, and color grading.
 */
function createPhotorealisticStudioComposite(
  ...
  <defs>
    ...
    <radialGradient id="rembrandtKey" cx="35%" cy="25%" r="75%">
      <stop offset="0%" stop-color="#ffffff" stop-opacity="0.15" />
      <stop offset="50%" stop-color="#fbbf24" stop-opacity="0.05" />
  ...
  <!-- Architectural Bokeh Circles simulating corporate suite window -->
  <circle cx="${width * 0.2}" cy="${height * 0.3}" r="${width * 0.25}" fill="#ffffff" opacity="0.04" />
  <circle cx="${width * 0.8}" cy="${height * 0.25}" r="${width * 0.3}" fill="#3b82f6" opacity="0.04" />
  <circle cx="${width * 0.5}" cy="${height * 0.15}" r="${width * 0.2}" fill="#f59e0b" opacity="0.04" />

  <!-- 2. User's REAL Photographic Face & Headshot Layer -->
  ${anchorUrl ? `
  <g id="userPhotoLayer">
    <image href="${anchorUrl}" x="0" y="0" width="${width}" height="${height}" preserveAspectRatio="xMidYMid slice" />
  </g>
  ` : `
  ...
  <text x="${width * 0.05}" y="${height * 0.94}" font-family="sans-serif" font-weight="800" font-size="24" fill="#ffffff" opacity="0.95" letter-spacing="1">
    AFRIHEADSHOT STUDIO
  </text>
  <text x="${width * 0.05}" y="${height * 0.97}" font-family="sans-serif" font-weight="500" font-size="15" fill="#e2e8f0" opacity="0.85">
