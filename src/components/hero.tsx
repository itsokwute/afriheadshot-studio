...
export const Hero: React.FC = () => {
  ...
  // 5-Row African Professional Photo Collage Dataset
  const row1 = [
  ...
  return (
    <section className={`relative overflow-hidden pt-12 pb-20 border-b transition-colors duration-300 ${
      ...
      {/* Background: Edge-to-Edge 5-Row Photo Collage Grid with Subtle Overlay */}
      <div className="absolute inset-0 opacity-20 pointer-events-none overflow-hidden select-none flex flex-col justify-between py-2 gap-3">
        <div className="flex space-x-3 -translate-x-10 animate-pulse">
          {row1.map((url, i) => (
            <div key={i} className="h-32 w-32 shrink-0 rounded-2xl overflow-hidden border border-slate-300/40 shadow-sm">
              <img src={url} alt="Executive Headshot" className="w-full h-full object-cover" />
            </div>
          ))}
        </div>
        <div className="flex space-x-3 translate-x-5">
          {row2.map((url, i) => (
            <div key={i} className="h-32 w-32 shrink-0 rounded-2xl overflow-hidden border border-slate-300/40 shadow-sm">
              <img src={url} alt="Executive Headshot" className="w-full h-full object-cover" />
            </div>
        ...
        <div className="flex space-x-3 -translate-x-14">
          {row3.map((url, i) => (
            <div key={i} className="h-32 w-32 shrink-0 rounded-2xl overflow-hidden border border-slate-300/40 shadow-sm">
              <img src={url} alt="Executive Headshot" className="w-full h-full object-cover" />
            </div>
      ...
      <div className={`absolute inset-0 z-0 ${
        isLight ? 'bg-slate-50/70 backdrop-blur-[2px]' : isTerracotta ? 'bg-[#FAF7F2]/70 backdrop-blur-[2px]' : 'bg-slate-950/75 backdrop-blur-[2px]'
      }`} />

      {/* Centered Foreground Container: Magic Studio Floating Card */}
      <div className="max-w-7xl mx-auto px-4 lg:px-8 relative z-10 pt-4">
        <div className={`max-w-xl mx-auto text-center p-8 sm:p-10 rounded-2xl border shadow-2xl transition-all ${
          ...
          {/* Trustpilot-Style Micro-Pill Badge */}
          <div className="inline-flex items-center space-x-2 bg-[#00b67a] text-white px-3.5 py-1.5 rounded-full text-xs font-bold shadow-sm mb-6">
            ...
            <span>Rated 4.9/5 with 16,800+ reviews across 25+ countries</span>
          </div>

          {/* Headline with Electric Blue Highlight */}
          ...
          <p className={`text-base sm:text-lg font-medium leading-relaxed mb-8 ${
            ...
            Free high-res AI generation. Preserving authentic melanin tones, afro textures, and tailored heritage attire.
          </p>

          {/* Solid Navy Primary CTA Button */}
          <button
            type="button"
